...
<style>
    ...
    .logo-quadrant {
        ...
        border: 2px solid #e2e8f0; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); height: 130px;
    }
    .premium-header-box {
        background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%);
        padding: 2.2rem; border-radius: 20px; box-shadow: 0 10px 25px -5px rgba(30, 58, 138, 0.3);
        text-align: center; border: 1px solid rgba(255,255,255,0.1); height: 130px;
    ...
    .stTextInput input, .stTextArea textarea, .stSelectbox div[data-baseweb="select"] { border: 2px solid #cbd5e1 !important; border-radius: 12px !important; background-color: #ffffff !important; color: #0f172a !important; font-weight: 500 !important; }
    ...
    .status-tag { display: inline-block; padding: 6px 16px; border-radius: 8px; font-size: 0.75rem; font-weight: 700; text-transform: uppercase; margin-bottom: 12px; }
...
with col_logo:
    st.markdown("""<div class="logo-quadrant"><div style="font-size: 3.8rem; text-align: center;">✏️</div></div>""", unsafe_allow_html=True)
...
if st.session_state.step == 1:
    ...
    if st.button("Avançar para Matriz Curricular ➔", type="primary", use_container_width=True):
        if not professor or not turmas:
            ...
            st.session_state.config = {'professor': professor, 'ano': ano, 'turmas': turmas, 'mes': mes_nome, 'periodo': periodo_texto, 'trimestre': trimestre_doc, 'quinzena': quinzen_label if 'quinzena_label' in locals() else quinzen_label}
            set_step(2); st.rerun()
